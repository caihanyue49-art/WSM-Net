...
if __name__ == "__main__":
    ...
    run = 1
    args.exp = args.encoder + '_EMCAD_kerne_sizes_MKDA-enhanced' + str(args.kernel_sizes) + '_dw_' + dw_mode + '_' + aggregation + '_lgag_ks_' + str(args.lgag_ks) + '_ef' + str(args.expansion_factor) + '_act_mscb_' + args.activation_mscb + '_loss_' + args.supervision + '_output_final_layer_Run'+str(run)+'_' + dataset_name + str(args.img_size)
    ...
    # =========================================================================
    # 【新增】: 在训练开始前，利用 thop 计算并打印 FLOPs 和 Params
    # =========================================================================
    from thop import profile
    from thop import clever_format

    print('Calculating FLOPs and Params...')
    # 构造一个与网络输入维度一致的 Dummy Input (通常为 BatchSize=1, Channels=3, H, W)
    dummy_input = torch.randn(1, 3, args.img_size, args.img_size).cuda()
    
    # 使用 thop 的 profile 计算
