...
class DEConv(nn.Module):
    def __init__(self, dim):
        ...
        self.conv1_5 = nn.Conv2d(dim, dim, 3, padding=1, groups=dim, bias=True) 
    ...
    def forward(self, x):
        ...
        w = w1 + w2 + w3 + w4 + w5
        b = b1 + b2 + b3 + b4 + b5
        res = nn.functional.conv2d(input=x, weight=w, bias=b, stride=1, padding=1, groups=x.shape[1])
...
class WAM(nn.Module):
    def __init__(
        ...
        self.ln_2 = nn.LayerNorm(hidden_dim)
        self.skip_scale2 = nn.Parameter(torch.ones(hidden_dim))
        self.conv = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True))
    def forward(self, x):
        x1 = self.conv(x)

        # --- 新增：处理奇数尺寸 ---
        B, C, H, W = x.shape
        # 如果高度或宽度是奇数，在右侧/底部填充 1 像素
        if H % 2 != 0 or W % 2 != 0:
