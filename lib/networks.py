...
class EMCADNet(nn.Module):
    def __init__(self, num_classes=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu', encoder='pvt_v2_b2', pretrain=True, pretrained_dir='./pretrained_pth/pvt/'):
        ...
        self.decoder = EMCAD(channels=channels, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add, lgag_ks=lgag_ks, activation=activation, num_classes=num_classes)
    ...
    def forward(self, x, mode='test'):
        ...
        # decoder
        predictions = self.decoder(x4, [x3, x2, x1])
        
        # prediction heads  
        p4, p3, p2, p1 = predictions
