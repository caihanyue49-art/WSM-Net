...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1, class_names=None):
    ...
    # 2. 确保颜色数量与标签数量一致 (不包含背景类，所以是 classes-1)
    # 直接截取列表，保证第1类永远是red，第2类永远是darkorange
    current_colors = my_colors[:classes-1]
    
    # 3. 如果 overlay_masks 需要 Hex 码 (十六进制)，则进行转换；如果支持颜色名，直接用 current_colors 也可以
    # 为了保险起见，我们将名字转为 HEX 码，且保持列表顺序不变
    cmaps = mcolors.CSS4_COLORS
    plot_colors = [cmaps[c] for c in current_colors]
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                # --- 修改开始 ---
                P = net(input)
                # 兼容处理：有些模型返回list（多尺度），有些直接返回Tensor
                ...
                fig_gt = overlay_masks(image[ind, :, :], masks, labels=mask_labels, colors=plot_colors, mask_alpha=0.5)
                fig_pred = overlay_masks(image[ind, :, :], preds_o, labels=mask_labels, colors=plot_colors, mask_alpha=0.5)
